...
namespace StereoSensors.Tracking
{
    class ESKF4 : ErrorStateKF
    {
        ...
        private Vector<double> p_offset;
        ...
        public ESKF4()
        {
            ...
            p_offset = Vector<double>.Build.Dense(3);
            ...
            Q.SetSubMatrix(0, 0, Matrix<double>.Build.SparseIdentity(state_size-3, state_size-3));// = Matrix<double>.Build.SparseIdentity(state_size, state_size);
            //Q = Matrix<double>.Build.Sparse(state_size, state_size) ;
            ...
            R[9, 9] = 0.0000003526;
            R[10, 10] = 0.0000003526;
            R[11, 11] = 0.0000003526;
            ...
            X_dx.SetSubMatrix(7, 6, Matrix<double>.Build.DiagonalIdentity(6));
        ...
        public override void InjectError()
        {
            ...
            q = LAExtensions.QuatFromAngleAxis(state[3], state[4], state[5]) * q;
            om_b[0] += state[6]; om_b[1] += state[7]; om_b[2] += state[8];
            p_offset[0] += state[9]; p_offset[1] += state[10]; p_offset[2] += state[11];

            //Debug.WriteLine("Offset: " + q_rot * p_offset);
            //Debug.WriteLine("Position: " + p);
        ...
        public override void UpdatePosterior(Vector<double> obs, double dt)
        {
            ...
            for (int i = 0; i < marker_refs.Count; i++)
            {
                ...
                H_x.SetSubMatrix(i * 3, 3, LAExtensions.QuaternionRotDerivative(q, marker_refs[i] + p_offset));
                H_x.SetSubMatrix(i * 3, 9, q_rot);
                obs_prediction.SetSubVector(i * 3, 3, q_rot * (marker_refs[i] + p_offset) + p);
            }
            ...
            H_x.SetSubMatrix(3 * marker_refs.Count, 3, LAExtensions.QuaternionRotDerivative(q, p_offset));
            H_x.SetSubMatrix(3 * marker_refs.Count, 9, q_rot);

            obs_prediction.SetSubVector(3 * marker_refs.Count, 3, p + q_rot*p_offset);
            Debug.WriteLine("Prediction: " + (p + q_rot * p_offset));
            Debug.WriteLine("Observation: " + obs.SubVector(12, 3)) ;
            Debug.WriteLine("Offset: " + q_rot * p_offset);
            Debug.WriteLine("Position: " + p);
        ...
        {
            Vector<double> p_state = Vector<double>.Build.Dense(9);
            p_state.SetSubVector(0, 3, p + q_rot * p_offset);
            p_state.SetSubVector(3, 3, p);
            p_state.SetSubVector(6, 3, p_offset);
