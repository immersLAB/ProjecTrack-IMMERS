...
namespace StereoSensors.Tracking
{
    class QuatESKF : ErrorStateKF
    {
        ...
        private int state_size = 6;
        
        // Markers
        private int obs_size = 12;
        ...
        public QuatESKF()
        {
            ...
            Q = Matrix<double>.Build.SparseIdentity(state_size, state_size);
            ...
            X_dx.SetSubMatrix(4, 3, Matrix<double>.Build.DiagonalIdentity(3));
        ...
        public override void InjectError()
        {
            ...
            q = LAExtensions.QuatFromAngleAxis(state[0], state[1], state[2]) * q;
            om_b[0] += state[3]; om_b[1] += state[4]; om_b[2] += state[5];
        ...
        public override void UpdatePosterior(Vector<double> obs, double dt)
        {
            ...
            X_dx[0, 0] = 0.5 * -q.ImagX; X_dx[0, 1] = 0.5 *  -q.ImagY; X_dx[0, 2] = 0.5 * -q.ImagZ;
            X_dx[1, 0] = 0.5 * q.Real; X_dx[1, 1] = 0.5 * q.ImagZ; X_dx[1, 2] = 0.5 * -q.ImagY;
            X_dx[2, 0] = 0.5 * -q.ImagZ; X_dx[2, 1] = 0.5 * q.Real; X_dx[2, 2] = 0.5 *  q.ImagX;
            X_dx[3, 0] = 0.5 * q.ImagY; X_dx[3, 1] = 0.5 * -q.ImagX; X_dx[3, 2] = 0.5 *  q.Real;
            ...
            for (int i = 0; i < marker_refs.Count; i++)
            {
                ...
                H_x.SetSubMatrix(i * 3, 0, LAExtensions.QuaternionRotDerivative(q, marker_refs[i]));
        ...
        public override void UpdatePrior(double dt)
        {
            ...
            //F.SetSubMatrix(0, 0, LAExtensions.RotationFromAngleAxis(-(om_m_t - om_b) * dt).Transpose());
            //F.SetSubMatrix(3, 6, Matrix<double>.Build.Diagonal(3, 3, -dt));
            F.SetSubMatrix(0, 3, -q_rot*dt);

            //LAExtensions.PrintMatrix(F);
            Q[0, 0] = dt * dt * var_om_nx * var_om_nx;
            Q[1, 1] = dt * dt * var_om_ny * var_om_ny;
            Q[2, 2] = dt * dt * var_om_nz * var_om_nz;

            Q[3, 3] = dt * var_om_wx * var_om_wx;
            Q[4, 4] = dt * var_om_wy * var_om_wy;
            Q[5, 5] = dt * var_om_wz * var_om_wz;
